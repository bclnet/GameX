...
using static OpenX.Gfx.Renderer;
...
public class GLMeshBufferCache {
    ...
    public int GetVertexArrayObject(IVBIB vbib, GfxShader shader, uint vtxIndex, uint idxIndex, uint baseVertex) {
...
public static class MeshBatchRenderer {
    ...
    static void DrawBatch(IEnumerable<MeshBatchRequest> drawCalls, Scene.RenderContext context) {
        ...
        foreach (var shaderGroup in groupedDrawCalls) {
            ...
            var uniformLocationAnimated = shader.UniformLocation("bAnimated");
            var uniformLocationAnimationTexture = shader.UniformLocation("animationTexture");
            var uniformLocationNumBones = shader.UniformLocation("fNumBones");
            var uniformLocationTransform = shader.UniformLocation("transform");
            var uniformLocationTint = shader.UniformLocation("m_vTintColorSceneObject");
            var uniformLocationTintDrawCall = shader.UniformLocation("m_vTintColorDrawCall");
            var uniformLocationTime = shader.UniformLocation("g_flTime");
            var uniformLocationObjectId = shader.UniformLocation("sceneObjectId");
            var uniformLocationMeshId = shader.UniformLocation("meshId");
            GL.UseProgram(shader.Program);
            GL.Uniform3f(shader.UniformLocation("vLightPosition"), 1, in cameraPosition);
            GL.Uniform3f(shader.UniformLocation("vEyePosition"), 1, in cameraPosition);
...
public class GLPickingTexture : IDisposable, IPickingTexture {
    ...
    public GfxShader Shader { get; }
    public GfxShader DebugShader { get; }
    ...
    public GLPickingTexture(OpenGLGfxModel gfxModel, ISource source, EventHandler<PickingResponse> onPicked) {
        (Shader, _) = gfxModel.ShaderManager.Create(source, "vrf.picking", new Dictionary<string, bool>()).Result;
        (DebugShader, _) = gfxModel.ShaderManager.Create(source, "vrf.picking", new Dictionary<string, bool>() { { "F_DEBUG_PICKER", true } }).Result;
        OnPicked += onPicked;
        Setup();
    }
...
public class GLRenderMaterial(MaterialShaderProp material) : RenderMaterial(material) {
    public override void Render(GfxShader shader) {
        // start at 1, texture unit 0 is reserved for the animation texture
        var textureUnit = 1U;
        int location;
        foreach (var texture in Textures) {
            location = shader.UniformLocation(texture.Key);
            if (location > -1) {
        ...
        switch (Material) {
            case MaterialShaderVProp p:
                foreach (var param in p.IntParams) {
                    location = shader.UniformLocation(param.Key);
        ...
        var alphaReference = shader.UniformLocation("g_flAlphaTestReference");
...
public class GLRenderableMesh(IOpenGfx[] gfx, ISource source, IMesh mesh, int meshIndex, IDictionary<string, string> skinMaterials = null, IEginModel model = null) : RenderableMesh(mesh, meshIndex, skinMaterials, model) {
    ...
    public override void SetRenderMode(string renderMode) {
        foreach (var s in DrawCallsOpaque.Union(DrawCallsBlended)) {
            ...
            (s.Shader, _) = GfxModel.ShaderManager.Create(Source, s.Shader.Name, parameters).Result;
    ...
    protected override void ConfigureDrawCalls(IDictionary<string, string> skinMaterials, bool firstSetup) {
        ...
        foreach (var sceneObject in data.GetArray("m_sceneObjects"))
            foreach (var objectDrawCall in sceneObject.GetArray("m_drawCalls")) {
                ...
                var (material, _) = GfxModel.MaterialManager.Create(Source, $"{materialName}_c").Result;
    ...
    void SetupDrawCallMaterial(DrawCall drawCall, IDictionary<string, bool> shaderArgs, RenderMaterial material) {
        ...
        // load shader
        (drawCall.Shader, _) = GfxModel.ShaderManager.Create(Source, drawCall.Material.Material.ShaderName, combinedShaderArgs).Result;
        // bind and validate shader
        GL.UseProgram(drawCall.Shader.Program);
        // tint and normal
        if (!drawCall.Material.Textures.ContainsKey("g_tTintMask")) drawCall.Material.Textures.Add("g_tTintMask", GfxModel.TextureManager.CreateSolid(1, 1, [1f, 1f, 1f, 1f]));
        if (!drawCall.Material.Textures.ContainsKey("g_tNormal")) drawCall.Material.Textures.Add("g_tNormal", GfxModel.TextureManager.CreateSolid(1, 1, [0.5f, 1f, 0.5f, 1f]));
    }
}
...
public class OctreeDebugRenderer<T> where T : class {
    ...
    public void Render(Camera camera, Pass pass) {
        if (pass == Pass.Translucent || pass == Pass.Both) {
            ...
            GL.UniformMatrix4f(Shader.UniformLocation("uProjectionViewMatrix"), 1, false, in projectionViewMatrix);
            GL.BindVertexArray(VaoHandle);
