...
namespace OpenX.Sfx.Al.Extensions.SOFT.SourceLatency;
...
public class SourceLatency : ALBase {
    ...
    static SourceLatency() => RegisterOpenALResolver();
    SourceLatency() { }
    ...
    public static unsafe void GetSource(int source, SourceLatencyVector2i param, out long value1, out long value2) { var values = stackalloc long[2]; GetSource(source, param, values); value1 = values[0]; value2 = values[1]; }

    public static unsafe void GetSource(int source, SourceLatencyVector2i param, Span<long> values) => GetSource(source, param, out values[0]);

    // FIXME: This might result in wrong values, though it seems to be somewhat correct...
    public static unsafe void GetSource(int source, SourceLatencyVector2i param, out int value1, out int value2, out long value3) { var values = stackalloc int[4]; GetSource(source, param, (long*)values); value1 = values[0]; value2 = values[1]; value3 = ((long*)values)[2]; }

    public static unsafe void GetSource(int source, SourceLatencyVector2d param, out double value1, out double value2) { var values = stackalloc double[2]; GetSource(source, param, values); value1 = values[0]; value2 = values[1]; }
    ...
    //public static void GetSource(int source, SourceLatencyVector2d param, out Vector2d values) { values.y = default; GetSource(source, param, out values.x); }
}
