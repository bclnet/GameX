...
public static class LibChd {
    ...
    [DllImport("chd_capi")] public static extern chd_error chd_open(IntPtr filename, int mode, IntPtr parent, out IntPtr chd);
    [DllImport("chd_capi")] public static extern void chd_close(IntPtr chd);
    [DllImport("chd_capi")] public static extern chd_error chd_read(IntPtr chd, uint hunknum, byte[] buffer);
    [DllImport("chd_capi")] public static extern chd_error chd_read_header(IntPtr filename, ref chd_header header);
    [DllImport("chd_capi")] public static extern chd_error chd_get_metadata(IntPtr chd, uint searchtag, uint searchindex, byte[] output, uint outputlen, out uint resultlen, out uint resulttag, out byte resultflags);
}
