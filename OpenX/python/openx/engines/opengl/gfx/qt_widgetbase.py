...
from openx.gfx import GfX, MouseState, KeyboardState
from openx.engines.opengl.egin import GLCamera, GLDebugCamera
...
class QOpenGLWidget(QOpenGLWidgetX):
    ...
    def tick(self, **kwargs) -> None:
        ...
        if not deltaTime: self.deltaTime = round((time.time() - self.elapsedTime) * 1000); self.elapsedTime = time.time()
