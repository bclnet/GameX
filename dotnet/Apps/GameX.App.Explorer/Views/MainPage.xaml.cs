...
public partial class MainPage : Window, INotifyPropertyChanged {
    ...
    public MainPage() {
        ...
        Engine.SelectedIndex = ((List<OpenX.Engine>)Engines)?.FindIndex(s => s.Id == Option.Engine) ?? -1;
    }
    ...
    public MainPage Open(Family family, IEnumerable<Uri> uris, string path = null) {
        var selected = (OpenX.Engine)Engine.SelectedItem;
        EngineX.Activate(selected);
    ...
    public void SetEngine(OpenX.Engine platform) {
        EngineX.Activate(platform);
        FileContent.SetEngine(platform);
    }

    IList<OpenX.Engine> _engines;
    public IList<OpenX.Engine> Engines {
        get => _engines;
        set { _engines = value; OnPropertyChanged(); }
    }
    ...
    void Engine_SelectionChanged(object sender, SelectionChangedEventArgs e) {
        var selected = (OpenX.Engine)Engine.SelectedItem;
        SetEngine(selected);
    }
