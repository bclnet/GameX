...
static class Store_Local {
    ...
    static Store_Local() {
        ...
        if (EngineX.PlatformOS == EngineX.OS.Android) gameRoots.Add(Path.Combine("/sdcard", GAMESPATH));
...
static class Store_WinReg {
    public static string GetPathByKey(string key, JsonElement elem)
        => EngineX.PlatformOS != EngineX.OS.Windows ? default
    ...
    static string GetPathByRegistryKey(string key, JsonElement? elem) {
        ...
        : elem.Value.TryGetProperty("path", out var z) ? Path.GetFullPath(EngineX.DecodePath(z.GetString(), path))
        : elem.Value.TryGetProperty("xml", out z) && elem.Value.TryGetProperty("xmlPath", out var y) ? GetSingleFileValue(EngineX.DecodePath(z.GetString(), path), "xml", y.GetString())
        : null;
    }
