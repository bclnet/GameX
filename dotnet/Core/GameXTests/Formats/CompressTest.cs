...
public class CompressTests {
    ...
    public async Task None(string arc, string sampleFile, int sampleFileSize) => await LoadDataAync(TestHelper.Paks[arc].Value, sampleFile, sampleFileSize);

    [TestMethod] // embeded: Doboz:DobozDecoder
    [DataRow("Red:Witcher3", "content0/bundles/xml.bundle:gameplay/items/def_item_misc.xml", 29362)] // 3
    public async Task DecompressDoboz(string arc, string sampleFile, int sampleFileSize) => await LoadDataAync(TestHelper.Paks[arc].Value, sampleFile, sampleFileSize);

    [TestMethod] // K4os.Compression.LZ4:LZ4Codec
    ...
    public async Task DecompressLz4(string arc, string sampleFile, int sampleFileSize) => await LoadDataAync(TestHelper.Paks[arc].Value, sampleFile, sampleFileSize);

    [TestMethod] // embeded: Compression:Lzf
    [DataRow("Tes:Witcher2", "base_scripts.dzip:core/2darray.ws", 968)]
    public async Task DecompressLzf(string arc, string sampleFile, int sampleFileSize) => await LoadDataAync(TestHelper.Paks[arc].Value, sampleFile, sampleFileSize);

    [TestMethod]
    [DataRow("Red:CP77", "", 0)] // 2
    public async Task DecompressOodleLZ(string arc, string sampleFile, int sampleFileSize) => await LoadDataAync(TestHelper.Paks[arc].Value, sampleFile, sampleFileSize);

    [TestMethod]
    [DataRow("Red:Witcher3", "", 0)] // 2
    public async Task DecompressSnappy(string arc, string sampleFile, int sampleFileSize) => await LoadDataAync(TestHelper.Paks[arc].Value, sampleFile, sampleFileSize);

    [TestMethod] // ICSharpCode.SharpZipLib:Zip.Compression.Streams:InflaterInputStream
    ...
    [TestMethod]
    public async Task DecompressZlib_2(string arc, string sampleFile, int sampleFileSize) => await LoadDataAync(TestHelper.Paks[arc].Value, sampleFile, sampleFileSize);

    //[TestMethod] // ICSharpCode.SharpZipLib:Lzw:Lzw​Input​Stream
    //[DataRow("Witcher2", "base_scripts.dzip:gameplay/abilities/monster_base_abl.xml", 593998)]
    //public async Task DecompressLzw​(string arc, string sampleFile, int sampleFileSize) => await LoadData(Helper.Paks[arc].Value, sampleFile, sampleFileSize);

    //[TestMethod] // embeded: SevenZip.Compression.LZMA:Decoder
    //[DataRow("Valve:Dota2", "dota/pak01_dir.vpk:gameplay/abilities/monster_base_abl.xml", 593998)]
    //public async Task DecompressLzma​(string arc, string sampleFile, int sampleFileSize) => await LoadData(Helper.Paks[arc].Value, sampleFile, sampleFileSize);

    [TestMethod]
    [DataRow("Tes:Fallout76", "SeventySix - 00UpdateMain.ba2:meshes/actors/moleminer/treasurehunter/treasurehunter.ztl", 17725)]
    public async Task Errors(string arc, string sampleFile, int sampleFileSize) => await LoadDataAync(TestHelper.Paks[arc].Value, sampleFile, sampleFileSize);
