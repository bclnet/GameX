...
public unsafe class Binary_UO : ArcBinary<Binary_UO> {
    ...
    Task ReadUop(BinaryAsset source, BinaryReader r) {
        ...
        var uopFile = Path.GetFileNameWithoutExtension(source.BinPath).ToLowerInvariant();
    ...
    Task ReadIdx(BinaryAsset source, BinaryReader r) {
        ...
        31 - animdata.mul
        */
        (string mulPath, int length, int fileId, Func<int, string> pathFunc) pair = source.BinPath switch {
        ...
        var mulPath = source.BinPath = pair.mulPath;
