...
public class Binary_Dds : IHaveMetaInfo, ITexture {
    ...
    public Binary_Dds(BinaryReader r, FileSource f, bool readMagic = true) {
        CapsReadDds = EngineX.Current.Caps.HasFlag(EngineX.Caps.ReadDds);
    ...
    public T Create<T>(string platform, Func<object, T> func) => func(CapsReadDds ? new TextureAsDds(Bytes) : new TextureAsBytes(Bytes, Format.value, Spans));
...
public unsafe class Binary_Img : IHaveMetaInfo, ITexture {
    ...
    public Binary_Img(BinaryReader r, FileSource f) {
        CapsDrawing = EngineX.Current.Caps.HasFlag(EngineX.Caps.Drawing);
        if (CapsDrawing) {
    ...
    public T Create<T>(string platform, Func<object, T> func) => func(new TextureAsBytes(Bytes, Format.value, null));
...
public class Binary_Raw : IHaveMetaInfo, ITexture {
    ...
    public T Create<T>(string platform, Func<object, T> func) => func(new TextureAsBytes([.. Body.SelectMany(s => PaletteData[s])], Format, null));
...
public unsafe class Binary_Xga(BinaryReader r, object tag) : IHaveMetaInfo, ITexture {
    ...
    public T Create<T>(string platform, Func<object, T> func) {
        ...
        return func(new TextureAsBytes(Type switch {
            1 => Decode1(),
