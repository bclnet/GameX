...
partial class ColladaFileWriter {
    ...
    void SetLibraryMaterials()
        ...
        => daeObject.Library_Materials = new Collada_Library_Materials {
            ...
            Material = File.Materials.Select(material => material.Name == null
                ? new Collada_Material {
                    ...
                    Instance_Effect = new Collada_Instance_Effect { URL = $"#{File.Name}-effect" }
                }
                : new Collada_Material {
                    Name = material.Name,
                    ID = $"{material.Name}-material", // this is the order the materials appear in the .mtl file. Needed for geometries.
                    Instance_Effect = new Collada_Instance_Effect { URL = $"#{material.Name}-effect" }
                }).ToArray(),
        };
}