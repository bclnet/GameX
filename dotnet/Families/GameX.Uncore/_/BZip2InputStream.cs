...
//        switch (currentState) {
//            case RAND_PART_B_STATE: SetupRandPartB(); break;
//            case RAND_PART_C_STATE: SetupRandPartC(); break;
//            case NO_RAND_PART_B_STATE: SetupNoRandPartB(); break;
//            case NO_RAND_PART_C_STATE: SetupNoRandPartC(); break;
...
//    void MakeMaps() {
//        nInUse = 0;
//        for (var i = 0; i < 256; ++i) if (inUse[i]) { seqToUnseq[nInUse] = (byte)i; unseqToSeq[i] = (byte)nInUse; nInUse++; }
//    }

//    void Initialize() {
//        char magic1 = BsGetUChar(), magic2 = BsGetUChar(), magic3 = BsGetUChar(), magic4 = BsGetUChar();
//        if (magic1 != 'B' || magic2 != 'Z' || magic3 != 'h' || magic4 < '1' || magic4 > '9') { streamEnd = true; return; }
//        SetDecompressStructureSizes(magic4 - '0');
//        computedCombinedCRC = 0;
//    }

//    void InitBlock() {
//        char magic1 = BsGetUChar(), magic2 = BsGetUChar(), magic3 = BsGetUChar(), magic4 = BsGetUChar(), magic5 = BsGetUChar(), magic6 = BsGetUChar();
//        if (magic1 == 0x17 && magic2 == 0x72 && magic3 == 0x45 && magic4 == 0x38 && magic5 == 0x50 && magic6 == 0x90) { Complete(); return; }
//        if (magic1 != 0x31 || magic2 != 0x41 || magic3 != 0x59 || magic4 != 0x26 || magic5 != 0x53 || magic6 != 0x59) { BadBlockHeader(); streamEnd = true; return; }
...
//    void FillBuffer() {
//        var thech = 0;
//        try { thech = baseStream.ReadByte(); }
//        catch (Exception) { CompressedStreamEOF(); }
...
//        for (var i = 0; i < 16; i++)
//            if (inUse16[i]) for (var j = 0; j < 16; j++) inUse[i * 16 + j] = BsR(1) == 1;
//            else for (var j = 0; j < 16; j++) inUse[i * 16 + j] = false;
...
//            for (var i = 0; i < alphaSize; i++) { maxLen = Math.Max(maxLen, len[t][i]); minLen = Math.Min(minLen, len[t][i]); }
...
//        var yy = new byte[256]; int nextSym;

//        int limitLast = BZip2Constants.BaseBlockSize * blockSize100k;
//        origPtr = BsGetIntVS(24);

//        RecvDecodingTables();
//        int EOB = nInUse + 1, groupNo = -1, groupPos = 0;
...
//        last = -1;
//        if (groupPos == 0) { groupNo++; groupPos = BZip2Constants.GroupSize; }

//        groupPos--;
//        int zt = selector[groupNo], zn = minLens[zt], zvec = BsR(zn), zj;

//        while (zvec > limit[zt][zn]) {
//            if (zn > 20) throw new Exception("Bzip data error"); // the longest code
//            zn++;
...
//            zvec = (zvec << 1) | zj;
//        }
//        if (zvec - baseArray[zt][zn] < 0 || zvec - baseArray[zt][zn] >= BZip2Constants.MaximumAlphaSize) throw new Exception("Bzip data error");
...
//                int s = -1; int n = 1;
//                do {
//                    if (nextSym == BZip2Constants.RunA) s += (0 + 1) * n;
...
//        for (var i = 0; i <= last; i++) { byte ch = ll8[i]; tt[cftab[ch]] = i; cftab[ch]++; }
//        //cftab = null;
//        tPos = tt[origPtr];
//        count = 0;
//        i2 = 0;
//        ch2 = 256;   /*-- not a char and not EOF --*/
//        if (blockRandomised) { rNToGo = 0; rTPos = 0; SetupRandPartA(); }
...
//    void SetupRandPartB() {
//        if (ch2 != chPrev) { currentState = RAND_PART_A_STATE; count = 1; SetupRandPartA(); }
//        else {
//            count++;
//            if (count >= 4) {
//                z = ll8[tPos];
...
//                SetupRandPartC();
//            }
//            else { currentState = RAND_PART_A_STATE; SetupRandPartA(); }
//        }
//    }

//    void SetupRandPartC() {
//        if (j2 < z) { currentChar = ch2; mCrc.Update(ch2); j2++; }
//        else { currentState = RAND_PART_A_STATE; i2++; count = 0; SetupRandPartA(); }
//    }

//    void SetupNoRandPartB() {
//        if (ch2 != chPrev) { currentState = NO_RAND_PART_A_STATE; count = 1; SetupNoRandPartA(); }
//        else {
//            count++;
//            if (count >= 4) { z = ll8[tPos]; tPos = tt[tPos]; currentState = NO_RAND_PART_C_STATE; j2 = 0; SetupNoRandPartC(); }
//            else { currentState = NO_RAND_PART_A_STATE; SetupNoRandPartA(); }
//        }
//    }

//    void SetupNoRandPartC() {
//        if (j2 < z) { currentChar = ch2; mCrc.Update(ch2); j2++; }
//        else { currentState = NO_RAND_PART_A_STATE; i2++; count = 0; SetupNoRandPartA(); }
//    }
...
//    static void BlockOverrun() => throw new Exception("BZip2 input stream block overrun");

//    static void BadBlockHeader() => throw new Exception("BZip2 input stream bad block header");

//    static void CrcError() => throw new Exception("BZip2 input stream crc error");

//    static void HbCreateDecodeTables(int[] limit, int[] baseArray, int[] perm, char[] length, int minLen, int maxLen, int alphaSize) {
//        int pp = 0;
//        for (int i = minLen; i <= maxLen; ++i)
//            for (int j = 0; j < alphaSize; ++j) if (length[j] == i) { perm[pp] = j; ++pp; }
...
//        var vec = 0;
//        for (var i = minLen; i <= maxLen; i++) { vec += (baseArray[i + 1] - baseArray[i]); limit[i] = vec - 1; vec <<= 1; }
