    ...
    internal static void Loaded(FamilyGame game, bool loadCell = true) {
        ...
        if (loadCell)
            try {
                ...
                Log.Error($"An exception occured while attempting to open {Cell} file. This needs to be corrected in order for Landblocks to load.");
        ...
        try {
            ...
            if (Portal.GetIteration() != ITERATION_PORTAL) Log.Info($"{Portal} iteration does not match expected end-of-retail version of {ITERATION_PORTAL}.");
        }
        catch (FileNotFoundException ex) {
            Log.Error($"An exception occured while attempting to open {Portal} file.");
            Log.Error($"Exception: {ex.Message}");
        }
        ...
        try {
            ...
            Log.Error($"An exception occured while attempting to open {Language} file.");
            Log.Error($"Exception: {ex.Message}");
        }
    }
#endif
