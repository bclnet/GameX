...
public static class UnityNifObjectBuilder {
    public static async Task<Object> BuildObject(ISource source, object path, bool isStatic, MaterialManager<Material, Texture2D> materialManager) {
        ...
        if (o.Roots.Length == 1) {
            ...
            foreach (var s in o.Roots) {
                var child = await InstantiateRootNiObject(name, source, isStatic, materialManager, s.Value);
    ...
    static async Task<Object> InstantiateRootNiObject(string name, ISource source, bool isStatic, MaterialManager<Material, Texture2D> materialManager, NiObject s) {
    ...
    static Task<Object> InstantiateNiObject(ISource source, bool isStatic, MaterialManager<Material, Texture2D> materialManager, NiObject s)
        => s switch {
    ...
    static async Task<Object> InstantiateNiNode(ISource source, bool isStatic, MaterialManager<Material, Texture2D> materialManager, NiNode s) {
    ...
    static async Task<Object> InstantiateRcnNode(ISource source, bool isStatic, MaterialManager<Material, Texture2D> materialManager, RootCollisionNode s) {
        var obj = new Object($"Root Collision Node{s.Name}");
        foreach (var t in s.Children)
            if (t != null)
                switch (t.Value) {
                    case NiTriShape z: (await InstantiateNiTriShape(source, isStatic, materialManager, z, false, true)).transform.SetParent(obj.transform, false); break;
    ...
    static async Task<Object> InstantiateNiTriShape(ISource source, bool isStatic, MaterialManager<Material, Texture2D> materialManager, NiTriShape s, bool visual, bool collidable) {
