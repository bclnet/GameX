...
def compute32(value: str) -> int:
    hash = 0xFFFFFFFF
    if value:
        for i in range(len(value)): hash = Table32[(hash&0xFF) ^ (ord(value[i])&0xFF)] ^ (hash >> 8)
...
def hashFilelist64(arc, entry) -> dict[int, str]:
    hashes = {}
    with arc.open(entry) as r:
        while True:
            line = r.readline().decode('utf-8')
