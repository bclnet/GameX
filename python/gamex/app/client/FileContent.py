...
class FileContent(QTabWidget):
    ...
    def setEngine(self, engine: object):
        id = engine.id if engine else 'UK'
        self.gfxWidget = \
            AppEginXWidget if id == 'EX' else \
            AppOpenGLWidget if id == 'GL' else \
            AppPanda3dWidget if id == 'PD' else \
            AppPyEngine3dWidget if id == 'P3' else \
            AppPygameWidget if id == 'PG' else \
            None #AppNullWidget
    ...
    def onInfo(self, item: MetaItem, source: ISource, infos: list[MetaInfo] = None):
        # dispose?
